...
function SuperMacroFrame_OnEvent(event)
	...
	if ( event=="VARIABLES_LOADED" ) then
		...
		SuperMacro_CreateSecureSuperMacroClicks();
	end
	if ( event=="PLAYER_ENTERING_WORLD" ) then
		SuperMacro_CreateSecureMacroClicks();
		SM_UpdateActionSpell();
	end
	...
	if ( event=="ADDON_ACTION_FORBIDDEN") then
		print( event, arg1, arg2, debugstack());
	end	
...
function SuperMacroFrame_SaveMacro()
	if ( SuperMacroFrame.textChanged and SuperMacroFrame.selectedMacro ) then
		local body = SuperMacroFrameText:GetText();
		local name = GetMacroInfo(SuperMacroFrame.selectedMacro, "name");
		EditMacro(SuperMacroFrame.selectedMacro, nil, nil, body);
		SuperMacroFrame.textChanged = nil;
		SM_UpdateActionSpell( name, "regular", body);
		-- secure action button
		local button = getglobal("MacroClick"..SuperMacroFrame.selectedMacro);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", SM_ReplaceAlias(body));
		local button = getglobal("MacroClick_"..name);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", SM_ReplaceAlias(body));
	end
end
...
function SuperMacroFrame_SaveSuperMacro()
	if ( SuperMacroFrame.textChanged and SuperMacroFrame.selectedSuper ) then
		...
		-- secure action button
		local button = getglobal("SuperMacroClick"..SuperMacroFrame.selectedSuper);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", SM_ReplaceAlias(macroBody));
		local button = getglobal("SuperMacroClick_"..macroName);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", SM_ReplaceAlias(macroBody));
	end
end
...
function FindSpell(spell, rank)
	...
	local s,r;
	local ys, yr;
	for k, book in pairs(booktype) do
		while spell do
...
function SM_UpdateAction()
	...
	local function doUpdate(button)
		if ( button ) then
			...
			local oldscript=button:GetScript("PreClick");
			button:SetScript("PreClick", function()
				if ( oldscript ) then
					oldscript();
				end
				if ( SM_ActionButton_OnClick ) then
					SM_ActionButton_OnClick();
				end
			end );
...
function GetSuperMacroInfo( superName, code)
	...
	if ( not code ) then
		return unpack(SM_SUPER[superName]);
	end
...
function GetOrderedSuperMacro( name )
	for i,v in pairs(SM_ORDERED) do
		if ( v==name ) then
			return i;
		end
	end
...
-- SM_ACTION_SPELL.super={}
	...
	-- update all macros
		for i=1, MACROS_REGULAR_SHOWN do
	...
	local id, book, texture, spell, count=FindFirstSpellOrItem(body);
	if ( not id ) then
	...
	if ( SM_ACTION_SPELL[macrotype][macroname].spell ~= spell ) then
		if ( count ) then
...
function SuperMacro_CreateSecureMacroClicks()
	for i=1, MACROS_REGULAR_SHOWN do
		local button;
		...
		if ( name ) then
			CreateFrame("Button", "MacroClick"..i, UIParent, "SecureActionButtonTemplate");
			button = getglobal("MacroClick"..i);
			button:SetAttribute("type", "macro");
			button:SetAttribute("macrotext", body);
			button:Hide();
		
			CreateFrame("Button", "MacroClick_"..name, UIParent, "SecureActionButtonTemplate");
			button = getglobal("MacroClick_"..name);
			button:SetAttribute("type", "macro");
			button:SetAttribute("macrotext", body);
			button:Hide();
		end
...
function SuperMacro_CreateSecureSuperMacroClicks()
	for i=1, GetNumSuperMacros() do
		local button;
		local name, texture, body = GetOrderedSuperMacroInfo(i);
		body = SM_ReplaceAlias(body);
		
		CreateFrame("Button", "SuperMacroClick"..i, UIParent, "SecureActionButtonTemplate");
		button = getglobal("SuperMacroClick"..i);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", body);
		button:Hide();
		
		CreateFrame("Button", "SuperMacroClick_"..name, UIParent, "SecureActionButtonTemplate");
		button = getglobal("SuperMacroClick_"..name);
		button:SetAttribute("type", "macro");
		button:SetAttribute("macrotext", body);
		button:Hide();
	end
