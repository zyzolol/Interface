...
SlashCmdList["SMOPTIONRUN"] = function(msg)
	local line = SM_ReplaceAlias(msg);
	line = SecureCmdOptionParse(line);
	if ( line ) then
		RunLine(line);
	end
end
...
function FindItem(item)
	...
	local link;
	local invslot = SM_INV_SLOT[strupper(item)];
	if ( invslot ) then
		return invslot, nil, GetInventoryItemTexture('player', invslot), GetInventoryItemCount('player', invslot);
	end				
	for i = 1,19 do
...
function Printd(...)
	for i=1, select("#", ...) do
		local t=select(i, ...) and (select(i, ...)~="" and select(i, ...) or '-""-' )or "-nil-";
