...
function ContainerFrameItemButton_OnModifiedClick(button, ignoreShift)
	...
	oldContainerFrameItemButton_OnModifiedClick(button, ignoreShift);
		end
		return;
	end
	oldContainerFrameItemButton_OnModifiedClick(button, ignoreShift);
...
function PaperDollItemSlotButton_OnModifiedClick(button)
	if ( button=="LeftButton" and SuperMacroFrame:IsVisible() ) then
		local link = GetInventoryItemLink("player", this:GetID());
		if ( not SuperMacroFrame_InsertItemText(link) ) then			
			oldPaperDollItemSlotButton_OnModifiedClick(button);
		end
		return;
	end
	oldPaperDollItemSlotButton_OnModifiedClick(button);
...
function SM_TradeSkillSkillButton_OnClick(button)
	...
	if ( link ) then
		...
		if ( SuperMacroFrame:IsVisible() and IsShiftKeyDown() ) then
			local skill=GetTradeSkillInfo(index); -- might be different than link, eg Smelt Copper instead of Copper Bar
			if ( IsControlKeyDown() ) then
				SuperMacroFrame_InsertItemText('"'..skill..'"');
			else
				SuperMacroFrame_InsertItemText(skill);
			end
			return;
...
local oldSpellButton_OnModifiedClick = SpellButton_OnModifiedClick;
function SpellButton_OnModifiedClick(drag)
	...
	oldSpellButton_OnModifiedClick(drag);
end